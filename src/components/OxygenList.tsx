...
import { Trash2, X, Check, Search, Wind, Info, MapPin, ArrowUpDown, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function OxygenList({ records }: OxygenListProps) {
  ...
  const [sortField, setSortField] = useState<'week' | 'location' | 'oxygenValue'>('week');
  ...
  const handleSort = (field: 'week' | 'location' | 'oxygenValue') => {
    if (sortField === field) {
  ...
  const sortedRecords = [...filteredRecords].sort((a, b) => {
    ...
    } else if (sortField === 'oxygenValue') {
      comparison = a.oxygenValue - b.oxygenValue;
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th 
                  onClick={() => handleSort('oxygenValue')}
                  className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100/50 transition-colors select-none group"
                >
                  <div className="flex items-center gap-1">
                    Kadar Oksigen
                    {sortField === 'oxygenValue' ? (
                      sortDirection === 'asc' ? <ChevronUp className="w-3.5 h-3.5 text-blue-600 font-bold" /> : <ChevronDown className="w-3.5 h-3.5 text-blue-600 font-bold" />
                    ) : (
                      <ArrowUpDown className="w-3 h-3 text-slate-300 group-hover:text-slate-400" />
                    )}
